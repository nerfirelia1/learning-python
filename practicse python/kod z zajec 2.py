...
def czywsrodku(Ax,Ay,Bx,By,Cx,Cy,Dx,Dy):
    AB=dl(Ax,Ay,Bx,By)
    AC=dl(Ax,Bx,Cx,Cy)
    BC=dl(Bx,By,Cx,Cy)
    ad=dl(Ax,Ay,Dx,Dy)
    ...
    cd=dl(Cx,Cy,Dx,Dy)
    i=(AB+AC+BC)/2
    p=sqrt(i*(i-AB)*(i-BC)*(i-AC))
    j=(AB+ad+bd)/2
    k=(AC+cd+ad)/2
    l=(BC+cd+bd)/2
    p2=sqrt(j*(j-ad)*(j-bd)*(j-AB))+ sqrt(k*(k-AC)*(k-cd)*(k-ad)) + sqrt(l*(l-BC)*(l-cd)*(l-bd))
    if p==p2:
        return "Tak"
    return "Nie"
print(czywsrodku(2,2,5,5,4,6,4,5))
print(czywsrodku(2,2,5,5,4,6,4,5))
print(sqrt(5-4))
